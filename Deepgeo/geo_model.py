...
class TGP(object):
    ...
    def text_layer(self, x, vsize, esize, fnum, maxlen, pool_window, config, text_embeddings):
        #variables
        emb = tf.get_variable("embedding", [vsize, esize], initializer=tf.random_uniform_initializer(-0.5/esize, 0.5/esize))
        ...
        word_inputs = text_embeddings
        char_inputs = tf.nn.embedding_lookup(emb, x)
            
        word_inputs_rev = tf.reverse(inputs, [1])
        #transform sent input from [batch_size,sent_len,hidden_size] to [sent_len,batch_size,hidden_size]
        word_inputs_s = [tf.squeeze(input_, [1]) for input_ in tf.split(word_inputs, maxlen, 1)]
        word_inputs_rev_s = [tf.squeeze(input_, [1]) for input_ in tf.split(word_inputs_rev, maxlen, 1)]

        char_inputs_rev = tf.reverse(inputs, [1])
        # transform sent input from [batch_size,sent_len,hidden_size] to [sent_len,batch_size,hidden_size]
        char_inputs_s = [tf.squeeze(input_, [1]) for input_ in tf.split(char_inputs, maxlen, 1)]
        char_inputs_rev_s = [tf.squeeze(input_, [1]) for input_ in tf.split(char_inputs_rev, maxlen, 1)]
        ...
        with tf.variable_scope("lstm-forward"):
            ...
            char_lstm_fw = tf.nn.rnn_cell.LSTMCell(esize, forget_bias=1.0, name='char_basic_lstm_cell')
            char_fw_outputs, _ = tf.nn.static_rnn(cell=char_lstm_fw, inputs=char_inputs_s,
                initial_state=char_lstm_fw.zero_state(config.batch_size, tf.float32))
            #insert zero state at the front and drop the last state [h_A, h_B, h_C] -> [0, h_A, h_B]
            char_fw_outputs.insert(0, zero_state)
            char_fw_outputs = char_fw_outputs[:-1]
        with tf.variable_scope("lstm-backward"):
            # lstm_bw = tf.nn.rnn_cell.BasicLSTMCell(esize, forget_bias=1.0)
            char_lstm_bw = tf.nn.rnn_cell.LSTMCell(esize, forget_bias=1.0, name='char_basic_lstm_cell')
            char_bw_outputs, _ = tf.nn.static_rnn(cell=char_lstm_bw, inputs=char_inputs_s,
                initial_state=char_lstm_bw.zero_state(config.batch_size, tf.float32))
            #reverse the time steps [j_C, j_B, j_A] -> [j_A, j_B, j_C]
            char_bw_outputs = tf.unstack(tf.reverse(tf.stack(char_bw_outputs), [0]))
            ...
            char_bw_outputs.append(zero_state)
            char_bw_outputs = char_bw_outputs[1:]


        with tf.variable_scope("lstm-forward"):
            # lstm_fw = tf.nn.rnn_cell.BasicLSTMCell(esize, forget_bias=1.0)
            word_lstm_fw = tf.nn.rnn_cell.LSTMCell(esize, forget_bias=1.0, name='word_basic_lstm_cell')
            word_fw_outputs, _ = tf.nn.static_rnn(cell=word_lstm_fw, inputs=word_inputs_s,
                initial_state=word_lstm_fw.zero_state(config.batch_size, tf.float32))
            #insert zero state at the front and drop the last state [h_A, h_B, h_C] -> [0, h_A, h_B]
            word_fw_outputs.insert(0, zero_state)
            word_fw_outputs = word_fw_outputs[:-1]
        with tf.variable_scope("lstm-backward"):
            # lstm_bw = tf.nn.rnn_cell.BasicLSTMCell(esize, forget_bias=1.0)
            word_lstm_bw = tf.nn.rnn_cell.LSTMCell(esize, forget_bias=1.0, name='word_basic_lstm_cell')
            word_bw_outputs, _ = tf.nn.static_rnn(cell=word_lstm_bw, inputs=word_inputs_rev_s,
                initial_state=word_lstm_bw.zero_state(config.batch_size, tf.float32))
            #reverse the time steps [j_C, j_B, j_A] -> [j_A, j_B, j_C]
            word_bw_outputs = tf.unstack(tf.reverse(tf.stack(word_bw_outputs), [0]))
            # bw_outputs = tf.unpack(tf.reverse(tf.pack(bw_outputs), [True, False, False]))
            #insert zero state at the end and drop the first state [j_A, j_B, j_C] -> [j_B, j_C, 0]
            word_bw_outputs.append(zero_state)
            word_bw_outputs = word_bw_outputs[1:]

        #reshape outputs from [sent_len,batch_size,hidden_size] to [batch_size,sent_len,hidden_size]
        char_fw_outputs = tf.reshape(tf.concat(axis=1, values=char_fw_outputs), [config.batch_size,-1,esize])
        char_bw_outputs = tf.reshape(tf.concat(axis=1, values=char_bw_outputs), [config.batch_size,-1,esize])

        # reshape outputs from [sent_len,batch_size,hidden_size] to [batch_size,sent_len,hidden_size]
        word_fw_outputs = tf.reshape(tf.concat(axis=1, values=word_fw_outputs), [config.batch_size, -1, esize])
        word_bw_outputs = tf.reshape(tf.concat(axis=1, values=word_bw_outputs), [config.batch_size, -1, esize])

        #concatenate the left right context and word embeddings [batch_size, sent_len, hidden_size*3]
        lrw_concat = tf.concat(axis=2, values=[char_fw_outputs, inputs, char_bw_outputs, word_fw_outputs, inputs, word_bw_outputs])
        # word_lrw_concat = tf.concat(axis=2, values=[word_fw_outputs, inputs, word_bw_outputs])
    ...
    def __init__(self, is_training, vocab_size, num_steps, num_classes, num_timezones, loc_vsize, desc_vsize, \
        ...
        #placeholders#
        ##############
        self.text_embeddings = tf.placeholder(tf.float32, [None, num_steps, config.text_emb_size])
        ...
        if config.text_filter_number > 0:
            with tf.variable_scope("text"):
                hidden_text, self.text_attn = self.text_layer(self.x, vocab_size, config.text_emb_size, \
                    config.text_filter_number, num_steps, config.text_pool_window, config, self.text_embeddings)
